...
class nginx
{
	...
	private function _createStandardErrorHandler()
	{
		if($this->settings['defaultwebsrverrhandler']['enabled'] == '1'
			...
			{
				$this->nginx_data[$vhosts_filename].= 'error_page 401 "' . $this->escapeConfigparameter($this->settings['defaultwebsrverrhandler']['err401']) . '";' . "\n";
			}

			if($this->settings['defaultwebsrverrhandler']['err403'] != '')
			{
				$this->nginx_data[$vhosts_filename].= 'error_page 403 "' . $this->escapeConfigparameter($this->settings['defaultwebsrverrhandler']['err403']) . '";' . "\n";
			}

			if($this->settings['defaultwebsrverrhandler']['err404'] != '')
			{
				$this->nginx_data[$vhosts_filename].= 'error_page 404 "' . $this->escapeConfigparameter($this->settings['defaultwebsrverrhandler']['err404']) . '";' . "\n";
			}
			
			if($this->settings['defaultwebsrverrhandler']['err500'] != '')
			{
				$this->nginx_data[$vhosts_filename].= 'error_page 500 "' . $this->escapeConfigparameter($this->settings['defaultwebsrverrhandler']['err500']) . '";' . "\n";
			}
		...
		while($row_ipsandports = $this->db->fetch_array($result_ipsandports))
		{
			...
			if($row_ipsandports['ssl'] == '1')
			{
				...
				if($row_ipsandports['ssl_cert_file'] != '')
				{
					...
					$this->nginx_data[$vhost_filename].= "\t" . 'ssl_certificate "' . $this->escapeConfigparameter(makeCorrectFile($row_ipsandports['ssl_cert_file'])) . '";' . "\n";
					$this->nginx_data[$vhost_filename].= "\t" . 'ssl_certificate_key "' . $this->escapeConfigparameter(makeCorrectFile($row_ipsandports['ssl_key_file'])). '";' .  "\n";
						
					if($row_ipsandports['ssl_ca_file'] != '')
					{
						$this->nginx_data[$vhost_filename].= 'ssl_client_certificate "' . $this->escapeConfigparameter(makeCorrectFile($row_ipsandports['ssl_ca_file'])) . '";' . "\n";
					}
				}
			}
			...
			if((int)$this->settings['phpfpm']['enabled'] == 1
				...
				$this->nginx_data[$vhost_filename].= "\t\t".'fastcgi_pass "unix:' . $this->escapeConfigparameter($php->getInterface()->getSocketFile()) . '";' . "\n";
			}
			else
			{
	...
	protected function create_pathOptions($domain)
	{
		...
		while($row = $this->db->fetch_array($result))
		{
			...
			{
				$path_options.= "\t".'error_page   404    "' . $this->escapeConfigparameter($row['error404path']) . '";' . "\n";
			}

			if(!empty($row['error403path']))
			{
				$path_options.= "\t".'error_page   403    "' . $this->escapeConfigparameter($row['error403path']) . '";' . "\n";
			}

			if(!empty($row['error500path']))
			{
				$path_options.= "\t".'error_page   502 503 504    "' . $this->escapeConfigparameter($row['error500path']) . '";' . "\n";
			}
...
//			{
				...
				if ($path == '/') {
					...
					$path_options.= "\t".'location "' . $this->escapeConfigparameter($path) . '" {' . "\n";
							...
							switch($single['path']){
								...
								default:
									if ($single['path']=='/'){
										...
										$path_options.= "\t\t" . 'auth_basic_user_file  "' . $this->escapeConfigparameter($single['usrf']) . '";'."\n";
...
//			}
			...
			&& $row['options_cgi'] != '0')
			{
				...
				$path_options.= "\t\t" . 'fastcgi_pass  "' . $this->escapeConfigparameter($this->settings['system']['perl_server']) . '";' . "\n";
			...
			foreach($htpasswds as $idx => $single)
			{
				...
				//{
					switch($single['path'])
					{
						...
						break;
						default:
							$path_options.= "\t" . 'location "' . $this->escapeConfigparameter($single['path']) . '" {' . "\n";
							$path_options.= "\t\t" . 'auth_basic            "Restricted Area";' . "\n";
							$path_options.= "\t\t" . 'auth_basic_user_file  "' . $this->escapeConfigparameter($single['usrf']) . '";'."\n";
							$path_options.= "\t".'}' . "\n";
					}
	...
	protected function composePhpOptions($domain, $ssl_vhost = false)
	{
		...
		if($domain['phpenabled'] == '1')
		{
			...
			$phpopts.= "\t\t".'fastcgi_pass "' . $this->escapeConfigparameter($this->settings['system']['nginx_php_backend']) . '";' . "\n";
	...
	protected function getWebroot($domain, $ssl)
	{
		...
		&& $this->settings['system']['deactivateddocroot'] != '')
		{
			...
			$webroot_text.= "\t".'root     "' . $this->escapeConfigparameter($this->settings['system']['deactivateddocroot']) . '";' . "\n";
			$this->_deactivated = true;
		}
		else
		{
			$webroot_text.= "\t".'root     "' . $this->escapeConfigparameter(makeCorrectDir($domain['documentroot'])) . '";' . "\n";
	...
	protected function getStats($domain,$single=array())
	{
		...
		&& $this->settings['system']['mod_log_sql'] != '1')
		{
			if($domain['parentdomainid'] == '0')
			{
				if($this->settings['system']['awstats_enabled'] == '1')
				{
					...
					$stats_text.= "\t\t" . 'alias "' . $this->escapeConfigparameter(makeCorrectFile($domain['customerroot'] . '/awstats/' . $domain['domain'])) . '";' . "\n";
					$stats_text.= "\t\t" . 'auth_basic            "Restricted Area";' . "\n";
					$stats_text.= "\t\t" . 'auth_basic_user_file  "' . $this->escapeConfigparameter($single['usrf']) . '";'."\n";
					$stats_text.= "\t" . '}' . "\n";
					$stats_text.= "\t" . 'location /awstats-icon {' . "\n";
					$stats_text.= "\t\t" . 'alias "' . $this->escapeConfigparameter(makeCorrectDir($this->settings['system']['awstats_icons'])) . '";' . "\n";
					$stats_text.= "\t" . '}' . "\n";
				}
				else
				{
					$stats_text.= "\t" . 'location /webalizer {' . "\n";
					$stats_text.= "\t\t" . 'alias "' .  $this->escapeConfigparameter(makeCorrectFile($domain['customerroot'] . '/webalizer/' . $domain['domain'])) . '";' . "\n";
					$stats_text.= "\t\t" . 'auth_basic            "Restricted Area";' . "\n";
					$stats_text.= "\t\t" . 'auth_basic_user_file  "' . $this->escapeConfigparameter($single['usrf']) . '";'."\n";
					$stats_text.= "\t" . '}' . "\n";
				}
			...
			else
			{
				...
				{
					$stats_text.= "\t" . 'location /awstats {' . "\n";
					$stats_text.= "\t\t" . 'alias "' . $this->escapeConfigparameter(makeCorrectFile($domain['customerroot'] . '/awstats/' . $domain['parentdomain'])) . '";' . "\n";
					$stats_text.= "\t\t" . 'auth_basic            "Restricted Area";' . "\n";
					$stats_text.= "\t\t" . 'auth_basic_user_file  "' . $this->escapeConfigparameter($single['usrf']) . '";'."\n";
					$stats_text.= "\t" . '}' . "\n";
					$stats_text.= "\t" . 'location /awstats-icon {' . "\n";
				...
				{
					$stats_text.= "\t" . 'location /webalizer {' . "\n";
					$stats_text.= "\t\t" . 'alias "' .  $this->escapeConfigparameter(makeCorrectFile($domain['customerroot'] . '/webalizer/' . $domain['parentdomain'])) . '";' . "\n";
					$stats_text.= "\t\t" . 'auth_basic            "Restricted Area";' . "\n";
					$stats_text.= "\t\t" . 'auth_basic_user_file  "' . $this->escapeConfigparameter($single['usrf']) . '";'."\n";
					$stats_text.= "\t" . '}' . "\n";
		...
		else
		{
			if($domain['customerroot'] != $domain['documentroot'])
			{
				...
				{
					$stats_text.= "\t" . 'location /webalizer {' . "\n";
					$stats_text.= "\t\t" . 'root "' .  $this->escapeConfigparameter(makeCorrectFile($domain['customerroot'] . '/webalizer/' . $domain['domain'])) . '";' . "\n";
					$stats_text.= "\t\t" . 'auth_basic            "Restricted Area";' . "\n";
					$stats_text.= "\t\t" . 'auth_basic_user_file  "' . $this->escapeConfigparameter($single['usrf']) . '";'."\n";
					$stats_text.= "\t" . '}' . "\n";
			...
			{
				$stats_text.= "\t" . 'location /awstats {' . "\n";
				$stats_text.= "\t\t" . 'alias "' . $this->escapeConfigparameter(makeCorrectFile($domain['documentroot'] . '/awstats/' . $domain['domain'])) . '";' . "\n";
				$stats_text.= "\t\t" . 'auth_basic            "Restricted Area";' . "\n";
				$stats_text.= "\t\t" . 'auth_basic_user_file  "' . $this->escapeConfigparameter($single['usrf']) . '";'."\n";
				$stats_text.= "\t" . '}' . "\n";
				$stats_text.= "\t" . 'location /awstats-icon {' . "\n";
	...
	protected function getLogFiles($domain)
	{
		...
		$logfiles_text.= "\t".'access_log    "' . $this->escapeConfigparameter($access_log) . '" combined;' . "\n";
		$logfiles_text.= "\t".'error_log    "' . $this->escapeConfigparameter($error_log) . '" error;' . "\n";
